{{-- resources/views/operadora/sanciones.blade.php --}}
@extends('layouts.app') {{-- si tienes layout de operadora, cámbialo aquí --}}
...
<div class="row mb-3">
    <div class="col-12">
        <h3 class="mb-1">MÓDULO SANCIONES</h3>
        <p class="text-muted mb-0">Aplicar y levantar sanciones (sin borrar historial).</p>
    </div>
</div>
    ...
    <div class="col-lg-5 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <button class="btn btn-danger w-100" id="btnSancionar" type="button">
                    Sancionar
                </button>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function cargarMoviles(query='') {
        ...
        .then(rows => {
            ...
            rows.forEach(r => {
                // buscador solo por mo_taxi, pero mostramos info para escoger
    ...
    function btnAcciones(r) {
        ...
        return `
            <button class="btn btn-sm btn-outline-success btn-levantar"
    ...
    // Buscar solo por número de móvil (mo_taxi)
    ...
    btnSancionar.addEventListener('click', function () {
        ...
        .then(({ok, j}) => {
            if (!ok || !j.success) {
                throw new Error(j.message || 'No se pudo registrar la sanción.');
            }
    ...
    btnConfirmLevantar.addEventListener('click', function () {
        ...
        .catch((e) => {
            showMsgLevantar('❌ ' + (e.message || 'Error'), 'alert alert-danger');
        })
        .finally(() => {
            btnConfirmLevantar.disabled = false;
        });
    });
